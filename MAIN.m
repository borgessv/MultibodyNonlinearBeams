...
t1 = 30; % End time of simulation [s]
...
if any(strcmp(model,'ROM')) || any(strcmp(model,'BOTH'))
    ...
    Jq_eq = lineariza_complexstep(@(q) element_position(q,DoF),xeq);
    [eigvec,eigval] = eig(K,(Jq_eq.'*M*Jq_eq));
    lambda = diag(eigval);
...
if any(strcmp(model,'BOTH')) && any(strcmp(animate,'Y'))
    animation(T,q_FOM,DoF,elem_type,'test_followerforce50_NACA','gif','avi',q_ROM)
end
