...
elseif isnumeric(c) && c ~= 100
    ...
    if exist('t','var')
        ...
        percentageOut = [percentageOut ' concluded  --  time remaining: ' num2str(h) 'h ' num2str(m) 'm ' num2str(floor(s)) 's'];
    else
        percentageOut = [percentageOut ' concluded'];
    end
...
elseif isnumeric(c) && c == 100
    ...
    if exist('t','var')
        ...
        [h,m,s] = hms(duration(0,0,t));
        percentageOut = [percentageOut ' concluded  --  total time: ' num2str(h) 'h ' num2str(m) 'm ' num2str(floor(s)) 's \n'];
        nDots = floor(c/100*strDotsMaximum);
        dotOut = [char(hex2dec('258F')) repmat(char(hex2dec('258E')),1,nDots) repmat(char(hex2dec('2005')),1,strDotsMaximum-nDots) char(hex2dec('258F')) ' '];
        strOut = [dotOut percentageOut];
    else
